...
export const TOAST_NOTICE_EVENT = "raple:toast-notice";
...
export function showToastNotice(title: string, description: string) {
  if (typeof window === "undefined") return;
  window.dispatchEvent(
    new CustomEvent<InfoNotice>(TOAST_NOTICE_EVENT, {
      detail: { title, description },
    }),
  );
}
